...
if __name__ == "__main__":
    ...
    with open("/home/rabbit/src/apicula/examples/attosocnodffe-tec0117.json") as f:
        pnr = json.load(f)

    bitmap = bslib.read_bitstream("/home/rabbit/src/apicula/examples/attosocnodffe-tec0117.fs")[0]
    bm = chipdb.tile_bitmap(db, bitmap)

    bels = get_bels(pnr)
    for bel in bels:
        ttype, row, col, num, parms, attrs, name = bel
        if ttype == 'SLICE' and int(parms['FF_USED']) != 0:
            ttype = parms['FF_TYPE']
        print(f'R{row}C{col}[{num}]', fse['header']['grid'][61][row - 1][col - 1], ttype)
