...
def main():
    ...
    parser.add_argument('bitstream_gowin')
    ...
    gw_bm = chipdb.tile_bitmap(db, gw_bitmap)
    ...
    print(f'({row}, {col}):{ttyp}')
    # bits w/o routing
    rbits = route_bits(db, row, col)
    r, c = np.where(gw_bm[(row, col)] == 1)
    gw_tile = set(zip(r, c))
    gw_bits = gw_tile - rbits
    ...
    if bits != gw_bits:
        diff = gw_bits ^ bits;
        print(sorted(diff))
        print(sorted(list(map(lambda df: get_fuse_num(ttyp, df[0] * 100 + df[1]), diff))))
        b_gw = bits - gw_bits
        print(f'bits - gw: {sorted(b_gw)}')
        print(sorted(list(map(lambda df: get_fuse_num(ttyp, df[0] * 100 + df[1]), b_gw))))
        gw_b = gw_bits - bits
        print(f'gw - bits: {sorted(gw_b)}')
        print(sorted(list(map(lambda df: get_fuse_num(ttyp, df[0] * 100 + df[1]), gw_b))))
    ...
    seen = set()
    for name, loc in cst.cells.items():
        row, col, side, bel = loc
        if (row, col) in seen:
            continue
        seen.update({(row, col)})
        row -= 1
