...
def fse_create_clocks(dev, device, dat: Datfile, fse):
    ...
    for i, ttyp in enumerate([85, 80, 81, 84]):
        # stop if chip has only 2 quadrants
        if i < 2 and device not in {'GW1N-9', 'GW1N-9C', 'GW2A-18', 'GW2A-18C'}:
            continue
        for row in range(dev.rows):
            for col in range(dev.cols):
            ...
            break
        extra_func = dev.extra_func.setdefault((row, col), {})
        dqce = extra_func.setdefault('dqce', {})
        for j in range(6):
            dqce[f'clkin{j}'] = f'SPINE{i * 8 + j}'
            dqce[f'ce{j}'] = ['A0', 'B0', 'C0', 'D0', 'A1', 'B1'][j]
