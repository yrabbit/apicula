...
class Device:
    ...
    # {wine_name: (type_name, base_type, multilier)}
    wire_delay: Dict[str, Tuple[str, str, int]] = field(default_factory=dict)
...
def fse_wire_delays(db):
    for i in range(33): # A0-D7
        db.wire_delay[wirenames[i]] = ("unk", "unk", 1)
    ...
    for i in range(56, 64): # X01-X08
        db.wire_delay[wirenames[i]] = ("X0", "X0", 1)
    db.wire_delay[wirenames[64]] = ("X1_0", "FX1", 1) # N100
    db.wire_delay[wirenames[65]] = ("X1", "unk", 1) # SN10
    db.wire_delay[wirenames[66]] = ("X1", "unk", 1) # N100
    for i in range(67, 71): # N130-E100
        db.wire_delay[wirenames[i]] = ("X1_0", "FX1", 1)
    for i in range(71, 73): # EW10-EW20
        db.wire_delay[wirenames[i]] = ("X1", "unk", 1)
    for i in range(73, 76): # E130-W130
        db.wire_delay[wirenames[i]] = ("X1_0", "FX1", 1)
    for i in range(76, 108): # N200-N270
        db.wire_delay[wirenames[i]] = ("X2_0", "X2", 1)
    for i in range(76, 108): # N200-W270
        db.wire_delay[wirenames[i]] = ("X2_0", "X2", 1)
    for i in range(108, 124): # N800-W830
        db.wire_delay[wirenames[i]] = ("X8_0", "X8", 1)
    for i in range(124, 127): # CLK0-CLK2
        db.wire_delay[wirenames[i]] = ("CLK", "unk", 1)
    for i in range(127, 130): # LSR0-LSR2
        db.wire_delay[wirenames[i]] = ("LSR", "unk", 1)
    for i in range(130, 133): # CE0-CE2
        db.wire_delay[wirenames[i]] = ("CE", "unk", 1)
    for i in range(133, 141): # SEL0-SEL7
        db.wire_delay[wirenames[i]] = ("SEL", "unk", 1)
    for i in range(141, 149): # N101-W131
        db.wire_delay[wirenames[i]] = ("X1_1", "unk", 1)
    for i in range(149, 181): # N201-W271
        db.wire_delay[wirenames[i]] = ("X2_1", "unk", 1)
    for i in range(181, 213): # N202-W272
        db.wire_delay[wirenames[i]] = ("X2_2", "unk", 2)
    for i in range(213, 229): # N804-W834
        db.wire_delay[wirenames[i]] = ("X8_1", "unk", 1)
    for i in range(229, 245): # N808-W838
        db.wire_delay[wirenames[i]] = ("X8_2", "unk", 2)
    for i in range(245, 253): # E110-N120
        db.wire_delay[wirenames[i]] = ("X1_0", "FX1", 1)
    for i in range(253, 261): # E111-N121
        db.wire_delay[wirenames[i]] = ("X1_0", "FX1", 1)
    for i in range(261, 269): # LB01-LB71
        db.wire_delay[wirenames[i]] = ("LW_BRANCH", "unk", 1)
    for i in range(269, 277): # GB00-GB70
        db.wire_delay[wirenames[i]] = ("GCLK_BRANCH", "unk", 1)
    db.wire_delay[wirenames[277]] = ("VCC", "unk", 1) # VSS
    db.wire_delay[wirenames[278]] = ("VSS", "unk", 1) # VCC
    for i in range(279, 285): # LT00-LT13
        db.wire_delay[wirenames[i]] = ("LW_TAP", "unk", 1)
    db.wire_delay[wirenames[285]] = ("LW_TAP_0", "unk", 1) # LT01
    db.wire_delay[wirenames[286]] = ("LW_TAP_0", "unk", 1) # LT04
    db.wire_delay[wirenames[287]] = ("LW_BRANCH", "unk", 1) # LTBO0
    db.wire_delay[wirenames[288]] = ("LW_BRANCH", "unk", 1) # LTBO1
    db.wire_delay[wirenames[289]] = ("LW_SPAN", "unk", 1) # SS00
    db.wire_delay[wirenames[290]] = ("LW_SPAN", "unk", 1) # SS40
    db.wire_delay[wirenames[291]] = ("GCLK_TAP", "unk", 1) # GT00
    db.wire_delay[wirenames[292]] = ("GCLK_TAP", "unk", 1) # GT10
    db.wire_delay[wirenames[293]] = ("GCLK_BRANCH", "unk", 1) # GBO0
    db.wire_delay[wirenames[294]] = ("GCLK_BRANCH", "unk", 1) # GBO1
    for i in range(295, 303): # DI0-DI7
        db.wire_delay[wirenames[i]] = ("DI", "unk", 1)
    for i in range(303, 309): # CIN0-CIN5
        db.wire_delay[wirenames[i]] = ("CIN", "unk", 1)
    for i in range(309, 314): # COUT0-COUT5
        db.wire_delay[wirenames[i]] = ("COUT", "unk", 1)
